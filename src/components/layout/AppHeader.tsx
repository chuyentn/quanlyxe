...
import { getExpiryStatus } from "@/utils/warningUtils";
...
export function AppHeader() {
  ...
  // Calculate generic alerts count
  ...
  const driverWarnings = drivers.filter(d => {
    const licenseStatus = getExpiryStatus((d as any).license_expiry || (d as any).license_expiry_date);
    const healthStatus = getExpiryStatus((d as any).health_check_expiry);
    return licenseStatus.status !== 'safe' || healthStatus.status !== 'safe';
  });

  const totalWarnings = vehicleWarnings.length + driverWarnings.length;
