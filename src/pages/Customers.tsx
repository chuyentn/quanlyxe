...
export default function Customers() {
  ...
  const handleAdd = () => {
    ...
    form.reset({
      customer_code: `KH-${format(new Date(), 'yyMM')}${Math.floor(Math.random() * 1000).toString().padStart(3, '0')}`,
      customer_name: "",
      customer_type: "Doanh nghiệp",
      short_name: "",
      tax_code: "",
  ...
  const handleImportData = async (rows: any[]) => {
    ...
    for (const row of rows) {
      try {
        await createMutation.mutateAsync({
          customer_code: String(row.customer_code || `KH-${Date.now()}`),
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input placeholder="KH-001" {...field} className="h-8" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
