...
const driverSchema = z.object({
  driver_code: z.string().min(1, "Mã tài xế là bắt buộc"),
  ...
  status: z.enum(['active', 'on_leave', 'inactive'] as const),
...
export default function Drivers() {
  ...
  const handleAdd = () => {
    ...
    form.reset({
      driver_code: `TX-${format(new Date(), 'yyMM')}${Math.floor(Math.random() * 1000).toString().padStart(3, '0')}`,
      full_name: "",
      phone: "",
      license_number: "",
      license_class: "",
  ...
  const handleExcelImport = async (rows: Record<string, unknown>[]) => {
    ...
    for (const row of rows) {
      try {
        await createMutation.mutateAsync({
          driver_code: String(row.driver_code || `TX-${Date.now()}`),
          ...
          status: (row.status as 'active' | 'on_leave' | 'inactive') || 'active',
  ...
  ], [handleDeleteClick, activeVehicles]);
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="VD: TX-006" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
