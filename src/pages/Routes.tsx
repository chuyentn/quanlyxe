...
const routeSchema = z.object({
  route_code: z.string().min(1, "Mã tuyến là bắt buộc"),
...
export default function Routes() {
  ...
  const handleAdd = () => {
    ...
    form.reset({
      route_code: `TD-${format(new Date(), 'yyMM')}${Math.floor(Math.random() * 1000).toString().padStart(3, '0')}`,
      route_name: "",
      origin: "",
      destination: "",
      distance_km: 0,
    ...
    for (const row of rows) {
      try {
        await createMutation.mutateAsync({
          ...row,
          route_code: String(row.route_code || `TD-${Date.now()}`),
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <FormField control={form.control} name="route_code" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl><Input placeholder="TĐ-001" className="h-8" {...field} /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
